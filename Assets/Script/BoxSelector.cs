...
{
    /*public event EventHandler<objectPointed> onHit;
    public class objectPointed : EventArgs
    {
        ...
        if (!GlobalVar.gameCompleted) {
            if (Input.GetKeyDown(InputSystem.keybinds["Click"]) && GlobalVar.TurnIndicator % GlobalVar.turnIndex.Count == GlobalVar.turnIndex["Player"]) {
                ...
                if (hit && raycastHit.transform.gameObject.GetComponent<BoxScript>().isEmpty()) {
                    //onHit?.Invoke(this, new objectPointed { hitObject = raycastHit.transform.gameObject }); // Can't use event, since order of execution is important in this case

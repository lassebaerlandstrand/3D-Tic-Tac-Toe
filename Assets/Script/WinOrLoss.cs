        ...
        for(int i = -1; i <= 0; i++) {
            for(int j = -1; j <= (i == 0 ? 0 : 1); j++) { // Since we fire raycast in in two directions, we only need to loop through half the possibilities
                for(int k = -1; k <= (i == 0 && j == 0 ? 0 : 1); k++) {
                    ...
                    RaycastHit[] raycastHits = Physics.RaycastAll(center + new Vector3(i, j, k) * length, new Vector3(-i, -j, -k), length * 2f * Mathf.Sqrt(3), layerMask); // Length of ray, and vector magnitude is not the same, so we need to multiply the length with sqrt(3)
                    ...
                    if (raycastHits.Length > 1) { // Hit something other than itself
                        ...
                        if (consecutivelyStatus == GlobalVar.dimensions) {
                            //Debug.Log(GlobalVar.nameOnStatus[boxStatus] + " won");
